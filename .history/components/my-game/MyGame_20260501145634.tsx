...
const MyGame: React.FC<MyGameProps> = ({ game }) => {
    ...
    const diamondSfxRef = useRef<HTMLAudioElement | null>(null);
    ...
    useEffect(() => {
        ...
        audio.preload = "auto";
        diamondSfxRef.current = audio;
        return () => {
            audio.pause();
            diamondSfxRef.current = null;
        };
    }, []);
    ...
    const playDiamondSfx = (): void => {
        if (isSfxMuted || !diamondSfxRef.current) return;
        const instance = diamondSfxRef.current.cloneNode(true) as HTMLAudioElement;
        instance.volume = diamondSfxRef.current.volume;
