...
const ExplosionSprite: React.FC<ExplosionSpriteProps> = ({
    ...
    const [fireFrameFailed, setFireFrameFailed] = useState(false);

    const totalFrames = Math.max(1, frameCount);
    const fireFrameStart = Math.floor(totalFrames * 0.75);
    const isFireFrameVisible = phase === "playing" && frame >= fireFrameStart;
    ...
    useEffect(() => {
        setFireFrameFailed(false);
        const image = new Image();
        image.onload = () => setFireFrameFailed(false);
        image.onerror = () => setFireFrameFailed(true);
    ...
    return (
        <span className="mines-explosion-sequence" data-phase={phase} data-fire-frame-visible={isFireFrameVisible ? "true" : "false"}>
            <img
                src="/my-game/bomb.svg"
                alt="Bomb"
            ...
            {!fireFrameFailed && (
                <img
                    ...
                    alt=""
                    aria-hidden="true"
                    className="mines-explosion-fire-frame"
                    draggable={false}
                />
            )}
        </span>
