...
const MyGame: React.FC<MyGameProps> = ({ game }) => {
    ...
    const handleRewatch = (): void => {
        ...
        if (explodedMine !== null) {
            ...
            // Win — play cashout sound before showing end card
            const winSfxId = setTimeout(() => {
                if (!isSfxMuted && cashoutSfxRef.current) {
                    cashoutSfxRef.current.currentTime = 0;
                    void cashoutSfxRef.current.play().catch(() => {});
                }
            }, delay);
            replaySequenceTimeoutsRef.current.push(winSfxId);
        }
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-4 sm:gap-8 lg:gap-10">
            ...
            >
                ...
                />
                {showEndCardGamesNote && endCardGamesCount > 0 && (modalPayout ?? 0) > 0 && (
                    <div className="absolute inset-0 z-40 flex items-center justify-center pointer-events-none">
                        <p className="hilo-end-card-games-note-inline" style={{ transform: "translateY(3rem)" }}>
                            (in {endCardGamesCount} game{endCardGamesCount !== 1 ? "s" : ""})
                        </p>
                    </div>
                )}
            </GameWindow>
