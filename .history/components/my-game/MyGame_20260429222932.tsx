...
const MyGame: React.FC<MyGameProps> = ({ game }) => {
    ...
    const buyAutoRoundsForTarget = (): void => {
        ...
        const purchaseText = `Bought ${roundsToBuy} auto rounds for ${buyCost.toFixed(2)} APE.`;

        if (betMode === "auto" && currentView === 2 && !isAutoBetting) {
            setCurrentView(0);
            setRoundState({
                ...INITIAL_ROUND_STATE,
                resultText: purchaseText,
            });
            setRevealAllTiles(false);
            setPendingManualRevealTiles([]);
            setManualShowAllAfterMine(false);
            manualClickOrderRef.current = {};
            manualClickCounterRef.current = 0;
