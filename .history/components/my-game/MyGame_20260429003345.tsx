...
const MANUAL_REVEAL_DELAY_MS = 340;
...
const MyGame: React.FC<MyGameProps> = ({ game }) => {
    ...
    const [pendingManualRevealTile, setPendingManualRevealTile] = useState<number | null>(null);
    ...
    const revealAllManualTiles =
        betMode === "manual" &&
        ((currentView === 1 && roundState.explodedMine !== null) ||
    ...
    const resetTimers = (): void => {
        ...
        if (manualRevealTimeoutRef.current !== null) {
            clearTimeout(manualRevealTimeoutRef.current);
            manualRevealTimeoutRef.current = null;
        }
        setPendingManualRevealTile(null);
    };
    ...
    const handleRevealTile = (index: number): void => {
        ...
        setPendingManualRevealTile(index);
    ...
    const handleToggleAutoTile = (index: number): void => {
        ...
        setAutoSelectedTiles((prev) => (
            prev.includes(index)
                ? prev.filter((tileIndex) => tileIndex !== index)
                : [...prev, index]
        ));
    };
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-4 sm:gap-8 lg:gap-10">
            <GameWindow
                ...
                startMinimizedResults={betMode === "auto"}
                disableBuiltInSong
            ...
            >
                <MyGameWindow
                    ...
                    pendingRevealTile={pendingManualRevealTile}
