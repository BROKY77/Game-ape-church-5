...
const MyGame: React.FC<MyGameProps> = ({ game }) => {
    ...
    const triggerWalletShake = useCallback((): void => {
    ...
    const requestRoundFromChain = useCallback(async (): Promise<ChainRoundData> => {
    ...
    const finishRound = useCallback((roundPayout: number, text: string): void => {
    ...
    }, [isAutoBetting, maxPayoutPerGame]);

    const startRound = useCallback(async (options?: { fromAuto?: boolean }): Promise<void> => {
        if (betAmount <= 0) {
            return;
        }
    ...
    }, [betAmount, mineCount, requestRoundFromChain, triggerWalletShake, walletBalance]);

    const handleRevealTile = useCallback((tileIndex: number): void => {
        if (
            currentView !== 1 ||
        ...
        roundState.isLoading,
        roundState.isResolving,
        roundState.minePositions,
        roundState.revealedTiles,
        roundState.safeReveals,
    ]);

    const handleCashOut = useCallback((): void => {
    ...
    }, [betAmount, currentView, finishRound, roundState.currentMultiplier, roundState.isLoading, roundState.isResolving, roundState.safeReveals]);

    const handleRandomPick = useCallback((): void => {
        if (currentView !== 1 || roundState.isLoading || roundState.isResolving) {
            return;
        }

        const unrevealedTiles = Array.from({ length: BOARD_TILE_COUNT }, (_, tileIndex) => tileIndex).filter(
            (tileIndex) => !roundState.revealedTiles.includes(tileIndex)
        );
        ...
        const nextIndex = Math.floor(Math.random() * unrevealedTiles.length);
        handleRevealTile(unrevealedTiles[nextIndex]);
    }, [currentView, handleRevealTile, roundState.isLoading, roundState.isResolving, roundState.revealedTiles]);
    ...
    return (
        <div className="flex flex-col gap-4 lg:flex-row lg:gap-8">
            ...
            <MyGameSetupCard
                ...
                gemsLeft={Math.max(0, BOARD_TILE_COUNT - mineCount - roundState.safeReveals)}
