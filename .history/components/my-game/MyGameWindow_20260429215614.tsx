...
const ExplosionSprite: React.FC<ExplosionSpriteProps> = ({
    ...
    const [fireBorderCounter, setFireBorderCounter] = useState(0);
    ...
    // Ping-pong: counter goes 0→(N-1)→0→(N-1)→... so no jump at loop boundary
    const firePingPongLength = (FIRE_FRAME_SPRITE_COUNT - 1) * 2; // 48
    const fireFrameIndex = fireBorderCounter <= FIRE_FRAME_SPRITE_COUNT - 1
        ? fireBorderCounter
        : firePingPongLength - fireBorderCounter;
    ...
    useEffect(() => {
        if (!showManualFireFrame || fireSpriteFailed) {
            setFireBorderCounter(0);
            return;
        }

        const msPerFrame = Math.max(16, Math.round(1000 / FIRE_FRAME_FPS));
        const pingPongLength = (FIRE_FRAME_SPRITE_COUNT - 1) * 2;
        const timer = window.setInterval(() => {
            setFireBorderCounter((prev) => (prev + 1) % pingPongLength);
        }, msPerFrame);
