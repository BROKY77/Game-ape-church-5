...
interface MyGameSetupCardProps {
    ...
    setAutoSelectedTiles: (tiles: number[]) => void;
...
const MyGameSetupCard: React.FC<MyGameSetupCardProps> = ({
    ...
    const toggleAutoTile = (index: number): void => {
        if (isAutoBetting || showStoppedAutoSummary) return;

        if (autoSelectedTiles.includes(index)) {
            setAutoSelectedTiles(autoSelectedTiles.filter((v) => v !== index));
            return;
        }

        setAutoSelectedTiles([...autoSelectedTiles, index]);
    };
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-5 gap-1.5">
                            {Array.from({ length: 25 }, (_, i) => i).map((idx) => {
                                const selected = autoSelectedTiles.includes(idx);
                                return (
                                    <button
                                        key={`auto-tile-${idx}`}
                                        type="button"
                                        onClick={() => toggleAutoTile(idx)}
                                        disabled={isAutoBetting || showStoppedAutoSummary}
                                        className="h-9 rounded-md text-xs font-semibold"
                                        style={{
                                            background: selected ? "#8CFF00" : "#2d3d47",
                                            color: selected ? "#0d1803" : "#d8eafa",
                                            border: selected ? "1px solid #8CFF00" : "1px solid #4b5d69",
                                            opacity: (isAutoBetting || showStoppedAutoSummary) ? 0.5 : 1,
                                            cursor: (isAutoBetting || showStoppedAutoSummary) ? "not-allowed" : "pointer",
                                        }}
                                    >
                                        {idx + 1}
                                    </button>
                        ...
                        <p className="mt-1 text-xs text-[#9FB2C0]">
                            Auto will click selected tiles in this order each round, then cash out.
                        </p>
                    </div>
                </div>
            )}
                ...
                {!stakeControlsDisabled && (
                    <div className="mt-1 mb-3">
                        ...
                        <div className="relative">
                            <button
                                type="button"
                                onClick={() => setIsMineListOpen((prev) => !prev)}
                                disabled={stakeControlsDisabled}
                                className="w-full px-3 py-2 rounded-lg text-sm text-left hilo-setup-input"
                                aria-expanded={isMineListOpen}
                                aria-haspopup="listbox"
                            >
                                <div className="flex items-center justify-between">
                                    <span>{mineCount}</span>
                                    <span className="text-[#9FB2C0]">▼</span>
                                </div>
                            </button>
                        ...
                        disabled={
                            betMode === "auto" && isAutoBetting
                                ...
                                : betMode === "manual" && currentView === 1
                                    ? !canCashOut
                                    : disabled || betAmount <= 0 || (betMode === "auto" && !canStartAutobet)
                        }
