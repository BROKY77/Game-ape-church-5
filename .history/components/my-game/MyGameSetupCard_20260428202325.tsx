...
const MyGameSetupCard: React.FC<MyGameSetupCardProps> = ({
    ...
    const getActionTileStyle = (action: RankAction): { background: string; color: string; border: string } => {
        if (action === "higher") {
            return { background: "#8CFF00", color: "#0d1803", border: "1px solid #8CFF00" };
        }
        if (action === "lower") {
            return { background: "#F59E0B", color: "#1C1300", border: "1px solid #F59E0B" };
        }
        return { background: "#334155", color: "#dbe9f4", border: "1px solid #64748B" };
    };
            ...
            <div className="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <input
                        ...
                        className="w-full px-2.5 py-1 rounded-lg text-sm hilo-setup-input"
                        style={{
                ...
                <div>
                    ...
                    <div className="relative">
                        <input
                            ...
                            disabled={isAutoBetting || showStoppedAutoSummary}
                            className="w-full pl-2.5 pr-7 py-1 rounded-lg text-sm hilo-setup-input"
                            style={{
                                color: (isAutoBetting || showStoppedAutoSummary) ? "rgba(255,255,255,0.25)" : undefined,
                                cursor: (isAutoBetting || showStoppedAutoSummary) ? "not-allowed" : "text",
                                opacity: (isAutoBetting || showStoppedAutoSummary) ? 0.5 : 1,
                ...
                <div className="mt-2 flex flex-col gap-2">
                    <div>
                        ...
                        <div className="mb-2 flex items-center gap-2 hilo-setup-label">
                            {([
                                { action: "higher" as const, label: "Higher/Same", style: { background: "#8CFF00", color: "#0d1803", border: "1px solid #8CFF00" } },
                                { action: "lower" as const, label: "Lower/Same", style: { background: "#F59E0B", color: "#1C1300", border: "1px solid #F59E0B" } },
                                { action: "skip" as const, label: "Skip", style: { background: "#334155", color: "#dbe9f4", border: "1px solid #64748B" } },
                                ...
                                return (
                                    <button
                                        ...
                                        className="px-2.5 py-1.5 rounded text-[10px] font-semibold leading-none hilo-setup-paint-btn"
                                        style={{
                                            ...style,
                        ...
                        <div className="grid grid-cols-7 gap-1">
                            {rankChoices.map((label, idx) => {
                                ...
                                const tileStyle = getActionTileStyle(action);
                                ...
                                return (
                                    <button
                                        ...
                                        disabled={!canApply}
                                            className="h-10 rounded-md flex flex-col items-center justify-center hilo-setup-rank-tile"
                                        style={{
                                            ...tileStyle,
                    ...
                    <div className="mt-3">
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                if (Number.isFinite(v)) setMineCount(Math.min(24, Math.max(1, v)));
                            }}
                            disabled={stakeControlsDisabled}
                            className="w-full px-3 py-2 rounded-lg text-sm hilo-setup-input"
                        />
                    ...
                    >
                        {betMode === "auto"
                            ...
                            : currentView === 1
                                ...
                                : hasActivePackage
                                    ? "Start Game"
                                    : `Start Game (${Math.max(1, liveManualRoundsForSummary)})`}
                    </Button>
                </div>
            </CardFooter>
        </Card>
