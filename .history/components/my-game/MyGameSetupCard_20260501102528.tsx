...
const MyGameSetupCard: React.FC<MyGameSetupCardProps> = ({
    ...
    const manualRoundsInputRef = React.useRef<HTMLInputElement>(null);
    const autoBetsInputRef = React.useRef<HTMLInputElement>(null);
    ...
    React.useEffect(() => {
        ...
        const handleWheel = (event: WheelEvent) => {
            ...
            setMineCount((prev) => Math.min(24, Math.max(1, prev + delta)));
        };
    ...
    }, [minesControlDisabled, setMineCount]);
    ...
    React.useEffect(() => {
        const manualInput = manualRoundsInputRef.current;
        ...
        manualInput.addEventListener("wheel", handleWheel, { passive: false });
        return () => manualInput.removeEventListener("wheel", handleWheel);
    }, [packageLocked, isAutoBetting, manualRoundsDraft, numberOfSpins]);

    React.useEffect(() => {
        const autoInput = autoBetsInputRef.current;
        if (!autoInput) return;
        ...
        autoInput.addEventListener("wheel", handleWheel, { passive: false });
        return () => autoInput.removeEventListener("wheel", handleWheel);
    }, [isAutoBetting, showStoppedAutoSummary, autoBetsDraft, autoBetCount]);
    ...
    const renderAutoSettings = (): React.ReactElement => (
        <div className="mt-4 flex flex-col gap-3">
            ...
            {showAutoProgressCards ? (
                ...
                <div className="px-3 py-[7px] rounded-lg hilo-setup-summary-box">
                    <p className={`text-sm font-bold text-white ${headingText}`}>
                        Select auto tiles directly in the game window.
                    </p>
                </div>
            )}
        </div>
