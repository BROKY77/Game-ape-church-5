...
const DIAMOND_SFX_PLAY_RATIO = 0.75;
const DIAMOND_SFX_FALLBACK_MS = 180;
...
const MyGame: React.FC<MyGameProps> = ({ game }) => {
    ...
    const diamondSfxStopTimersRef = useRef<Map<HTMLAudioElement, ReturnType<typeof setTimeout>>>(new Map());
    ...
    useEffect(() => {
        ...
        return () => {
            diamondSfxStopTimersRef.current.forEach((timeoutId) => clearTimeout(timeoutId));
            diamondSfxStopTimersRef.current.clear();
    ...
    const playDiamondSfx = (): void => {
        ...
        const existingStopTimeout = diamondSfxStopTimersRef.current.get(audio);
        if (existingStopTimeout) {
            clearTimeout(existingStopTimeout);
            diamondSfxStopTimersRef.current.delete(audio);
        }
        ...
        const durationMs = Number.isFinite(audio.duration) && audio.duration > 0
            ? audio.duration * 1000 * DIAMOND_SFX_PLAY_RATIO
            : DIAMOND_SFX_FALLBACK_MS;
        ...
        diamondSfxStopTimersRef.current.set(audio, stopTimeout);
    };
