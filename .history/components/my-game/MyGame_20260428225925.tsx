...
const MyGame: React.FC<MyGameProps> = ({ game }) => {
    ...
    const [showAllAfterManualCashout, setShowAllAfterManualCashout] = useState(false);
    ...
    const revealAllManualCashoutTiles =
        betMode === "manual" &&
        (currentView === 2 || (currentView === 1 && roundState.explodedMine !== null)) &&
        showAllAfterManualCashout;
    const revealAllTiles = revealAllAutoTiles || revealAllManualCashoutTiles;
    ...
    const playGame = async (options?: {
        ...
        setShowAllAfterManualCashout(false);
        setCurrentView(0);
        resetTimers();
    ...
    const handleRevealTile = (index: number): void => {
        ...
        if (isMine) {
            ...
            setShowAllAfterManualCashout(true);
